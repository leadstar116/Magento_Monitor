...
if (typeof(BASE_URL) !== 'undefined') {
	var mediaImageFolder = BASE_URL + 'pub/media/mgs/fbuilder/images/';
}else{
	var curentUrl = window.require.s.head.baseURI;
	var webBaseUrl = curentUrl.split('fbuilder/');
...
require([
	...
	(function(mgsjQuery){
	  ...
	  mgsjQuery.fn.mColorPicker.drawPickerTriggers = function () {
		...
		mgsjQuery(img).attr({
		  'src': mediaImageFolder + 'color.png'
		}).css({
		  'border': 0,
		  'margin': '0 0 0 3px',
	  ...
	  mgsjQuery.fn.mColorPicker.drawPicker = function () {
		...
		mgsjQuery(div).attr({
		  ...
		  'border': 0,
		  'cursor': 'crosshair',
		  'background-image': 'url(' + mediaImageFolder + 'picker.png)'
		}).appendTo($w);
		...
		$f.attr({
		  ...
		  'background-image': 'url(' + mediaImageFolder + 'grid.gif)',
		  'position': 'relative',
		  'height': '26px'
		}).appendTo($w);
	  ...
	  mgsjQuery.fn.mSetInputColor = function (color) {
		...
		var $t = mgsjQuery(this),
			css = {
			  ...
			  'background-image': (color == 'transparent')? "url('" + mediaImageFolder + "grid.gif')": '',
